...
fn main() {
    ...
    let mut bounds = Vec::<(usize, usize, Vec<(usize, usize)>)>::new();
    let mut i = 0;
    while i < data.len() {
        let mut j = i + 1;
        while j < data.len() {
    ...
    let mut res = vec![(0, 0); 11];
    for i in 0..bounds.len() {
        res[10].0 += bounds[i].2[10].0;
        res[10].1 += bounds[i].2[10].1;
    }

    // Print.

    let n = res[10].0 + res[10].1;
    let nznz = 100.0 * res[10].0 as f64 / n as f64;
    println!("\n{nznz:.1}%");
