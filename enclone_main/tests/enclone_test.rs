...
fn run_test(it: usize, test: &str, testname: &str, ok: &mut bool, logx: &mut String) {
...
fn test_enclone() {
    ...
    results.par_iter_mut().for_each(|res| {
        ...
        run_test(it, &test, "test", &mut res.1, &mut res.2);
    });
...
fn test_extended() {
    ...
    results.par_iter_mut().for_each(|res| {
        ...
        run_test(it, &test, "ext_test", &mut res.1, &mut res.2);
    });
    for i in 0..results.len() {
        print!("{}", results[i].2);
        if !results[i].1 {
