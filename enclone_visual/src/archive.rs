...
pub fn archive(slf: &mut gui_structures::EncloneVisual) -> Element<Message> {
    let archive_title = Text::new(&format!("Archive")).font(LIBERATION_SANS).size(30);
    let archive_snapshot_button = Button::new(
        &mut slf.archive_snapshot_button,
        Text::new("Snapshot").font(LIBERATION_SANS).color(slf.archive_snapshot_button_color),
    )
    .on_press(Message::ArchiveSnapshot);
    let archive_close_button = Button::new(&mut slf.archive_close_button, Text::new("Dismiss").font(LIBERATION_SANS))
    ...
    let refresh_button = Button::new(
        ...
        Text::new("Refresh").font(LIBERATION_SANS).color(slf.archive_refresh_button_color),
    )
    ...
    let text0 =
        Text::new("enclone visual can save sessions to the directory ~/enclone/visual/history.").font(LIBERATION_SANS);
    let text1 = Text::new(
    ...
    let text2 =
        Text::new("▒ expand - Display the commands in a saved session by checking the expand box.").font(LIBERATION_SANS);
    let text3 = Text::new(
    ...
    let text4 = Text::new("▒ delete - Delete a saved session by checking the delete box.").font(LIBERATION_SANS);
    let text5 = Text::new(
    ...
    if sharing {
        ...
        if valids > 0 {
            let row = Row::new()
                ...
                .push(Text::new("           check to complete share").font(LIBERATION_SANS).size(16))
                .push(Space::with_width(Units(14)))
                .push(Checkbox::new(slf.do_share, "", Message::DoShare))
                .push(Text::new("or uncheck share to cancel").font(LIBERATION_SANS).size(16));
            share_body = share_body.push(Space::with_height(Units(8)));
            share_body = share_body.push(row);
...
.font(LIBERATION_SANS)
    ...
    for (i, y, narbut, copynarbut) in izip!(
        ...
        let copy_cookbook_narrative_button = Button::new(
            copynarbut,
            Text::new("Copy").font(LIBERATION_SANS).color(slf.copy_cookbook_narrative_button_color[i]),
        )
        .on_press(Message::CopyCookbookNarrative(i));
        let mut row = Row::new()
            .push(Text::new("    ").font(DEJAVU))
            .push(Button::new(narbut, Text::new(&log).font(LIBERATION_SANS)).on_press(Message::CopyCookbookNarrative(i)));
...
.font(LIBERATION_SANS)
            ...
            row = row.push(
                Button::new(
                    q,
                    Text::new("Apply").font(LIBERATION_SANS).color(slf.archive_name_change_button_color[i]),
                )
            ...
            let copy_narrative_button = Button::new(
                copynarbut,
                Text::new("Copy").font(LIBERATION_SANS).color(slf.copy_archive_narrative_button_color[i]),
            )
            .on_press(Message::CopyArchiveNarrative(i));
            let mut row = Row::new()
                .push(Text::new("    ").font(DEJAVU))
                .push(Button::new(narbut, Text::new(&log).font(LIBERATION_SANS)).on_press(Message::ArchiveNarrative(i)));
