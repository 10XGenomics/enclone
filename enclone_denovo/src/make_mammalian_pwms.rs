...
pub fn make_mammalian_pwms() -> Vec<(String, String, usize, Vec<Vec<(u32, u8)>>)> {
    ...
    for m in 0..refs_all.len() {
        ...
        for i in 0..refs.len() {
            ...
            */
            ...
            calls.push((chain_type.to_string(), "cdr1".to_string(), cdr1.len(), cdr1.clone()));
            calls.push((chain_type.to_string(), "cdr2".to_string(), cdr2.len(), cdr2.clone()));
            calls.push((chain_type.to_string(), "fwr1".to_string(), fwr1.len(), fwr1.clone()));
            calls.push((chain_type.to_string(), "fwr2".to_string(), fwr2.len(), fwr2.clone()));
            calls.push((chain_type.to_string(), "fwr3".to_string(), fwr3.len(), fwr3.clone()));
        }
    }
    calls.sort();
    ...
    for s in starts.iter() {
        ...
        for (i, len) in s.1.iter().enumerate() {
            ...
            let hlen = *len - 1;
            let m1 = calls.
                lower_bound_by_key(&(chain_type, feature, &llen), |(a, b, c, _d)| (a, b, c));
            let m2 = calls.
                upper_bound_by_key(&(chain_type, feature, &hlen), |(a, b, c, _d)| (a, b, c));
