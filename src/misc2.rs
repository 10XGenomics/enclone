...
pub fn create_exact_subclonotype_core(
    // inputs:
    tig_bc: &Vec<Vec<TigData>>, r: usize, s: usize, to_delete: &Vec<bool>,
    // outputs:
    share: &mut Vec<TigData1>, clones: &mut Vec<Vec<TigData0>>,
) {
    for m in 0..tig_bc[r].len() {
        ...
        loop {
            ...
            for t in r..s {
                if !to_delete[t-r] 
                    && tig_bc[t][m].j_stop + pos < tig_bc[t][m].full_seq.len() {
...
pub fn find_exact_subclonotypes( 
    ...
    while r < tig_bc.len() {
        ...
        study_consensus( &mut _count, &ctl, &share, &clones, &exact_clonotypes, &refdata );
        ...
        if clones.len() > 0 {
            exact_clonotypes.push( ExactClonotype { share: share, clones: clones } );
    ...
    if !ctl.silent {
        println!( "found {} exact subclonotypes from {} productive pairs",
            exact_clonotypes.len(), tig_bc.len() );
        println!( "max exact subclonotype size = {}", max_exact);
    }
    if ctl.comp {
        println!( "used {:.1} seconds finding exact subclonotypes, peak mem = {:.2} GB", 
            elapsed(&texact), peak_mem_usage_gb() );
    }
    if ctl.gen_opt.fasta.len() > 0 {
        ...
        for i in 0..exact_clonotypes.len() {
            ...
            for j in 0..x.share.len() {
                fwriteln!( f, ">exact_clonotype{}.chain{}.VJ\n{}", 
                    i, j+1, strme(&x.share[j].seq) );
            }
        }
    }
    if ctl.gen_opt.exact.is_some() {
        ...
        for i in 0..ex.share.len() {
            ...
            println!( "chain {} = {} + {} = {}", i+1, 
                refdata.name[vid], refdata.name[jid], ex.share[i].cdr3_aa );
        }
        for i in 0..ex.clones.len() {
            let x = &ex.clones[i][0];
            println!( 
                "clone {} = {}.{}", i+1, ctl.sample_info.dataset_id[x.lena_index], x.barcode );
        }
        println!("");
    }
    exact_clonotypes
    ...
    if ctl.gen_opt.indels {
        ...
        for i in 0..tig_bc.len() {
            for j in 0..tig_bc[i].len() {
                let x = &tig_bc[i][j];
                cs.push( ((x.cdr3_dna.clone(),x.v_ref_id), x.seq.len(), x.cdr3_aa.clone()) );
            }
        }
        unique_sort(&mut cs);
        let mut i = 0;
    ...
    if ctl.gen_opt.no_reuse {
        ...
        while i < reuse.len() {
            ...
            if frac >= MIN_REUSE_FRAC_TO_SHOW {
                ...
                eprintln!( "{}, {} ==> {} of {}, {} ({:.1}%)", 
                    ctl.sample_info.dataset_id[l1], ctl.sample_info.dataset_id[l2], n, n1, n2,
                    100.0 * frac );
            }
            i = j;
        }
