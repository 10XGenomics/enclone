...
pub fn main_enclone( args: &Vec<String> ) {
    ...
    if ctl.gen_opt.ext.len() > 0 {
        ...
        for line in f.lines() {
            ...
            ctl.gen_opt.extc.insert( ( fields[0].to_string(), fields[1].to_string() ), 
                fields[2].to_string() );
    ...
    if ctl.comp {
        println!( "used {:.2} seconds building reference, peak mem = {:.2} GB", 
            elapsed(&tr), peak_mem_usage_gb() );
    }
    if ctl.comp {
        println!( "used {:.2} seconds up through building reference", elapsed(&tall) );
    }
    ...
    if ctl.gen_opt.dref_file.len() > 0 {
        ...
        for i in 0..alt_refs.len() {
            ...
            fwriteln!( f, ">{}.{}.{}, {}\n{}", refdata.id[ref_id], donor+1, count+1, 
                refdata.name[ref_id], alt_seq.to_string() );
            count += 1;
        }
    }
    ...
    print_clonotypes( &refdata, &drefs, &ctl, &exact_clonotypes, &info, &eq, &gex_info );
    if ctl.comp {
    ...
    if ctl.comp {
        println!( "{:.2} seconds total, peak mem = {:.2} GB", elapsed(&tall), peak_mem_usage_gb() );
    }
    println!("");
    std::process::exit(0);
}
