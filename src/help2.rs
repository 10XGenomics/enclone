...
pub fn help2() {
    ...
    */
    ...
    if ( args.len() == 3 && args[1] == "help" && args[2] == "example1" ) || help_all {
        ...
        println!( "{}", "This shows an invocation of enclone that takes one dataset as input \
        ...
        print( "The view you see here is configurable: see the documentation at \
    ...
    if ( args.len() == 3 && args[1] == "help" && args[2] == "example2" ) || help_all {
        ...
        print( "This shows an invocation of enclone that takes VDJ, gene expression and feature \
    ...
    if ( args.len() == 3 && args[1] == "help" && args[2] == "support" ) || help_all {
        ...
        print( "3. If you're sure that enclone made a mistake.  Usually an actionable mistake is \
            ...
            our best!\n\n" );
        print( "\\red{Please communicate with us by writing to} \
        ...
        print( "If you are github-savvy, we also welcome github issues if applicable and you \
            ...
            author on the site.\n\n" );
        print( "\\blue{Before writing to us, please check our faq by typing} \
    ...
    if ( args.len() == 3 && args[1] == "help" && args[2] == "input" ) || help_all {
        ...
        print( "Both input forms involve abbreviated names (discussed below), which should be as \
            short as possible, as longer abbreviations will increase the width of the clonotype \
            displays.\n\n" );
        print( "\\boldred{█ 1 █} To point directly at input files on the command line, use e.g.\n\
        ...
        print( "Using this input system, each dataset is assigned an abbreviated name, which is \
            everything \
        ...
        print( "Matching gene expression and/or feature barcode data may also be supplied using \
        ...
        end_escape!();
        print( "This file should be a CSV (comma-separated values) file, with one line per cell \
        ...
        rows.push( vec![ "\\hline".to_string(), "\\hline".to_string(), "\\hline".to_string() ] );
        rows.push( vec![ "tcr or bcr".to_string(), "(required!)".to_string(), 
        ...
        rows.push( vec![ "".to_string(), "".to_string(), "must be used".to_string() ] );
        rows.push( vec![ "gex".to_string(), "null".to_string(), 
            "path to GEX dataset, which may include or consist entirely".to_string() ] );
        rows.push( vec![ "".to_string(), "".to_string(), "of FB data".to_string() ] );
        rows.push( vec![ "\\hline".to_string(), "\\hline".to_string(), "\\hline".to_string() ] );
        rows.push( vec![ "sample".to_string(), "s1".to_string(), 
            "abbreviated name of sample".to_string() ] );
        rows.push( vec![ "\\hline".to_string(), "\\hline".to_string(), "\\hline".to_string() ] );
        rows.push( vec![ "donor".to_string(), "d1".to_string(), 
