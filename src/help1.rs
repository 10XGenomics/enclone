...
pub fn help1( args: &Vec<String> ) {
    ...
    if ( args.len() == 2 && args[1] == "help" )
        || ( args.len() == 2 && args[1] == "--help" ) || help_all {
        if help_all {
            println!("");
        }
        begin_doc!("");
    ...
    if args.len() == 1 
        || ( args.len() == 3 && args[1] == "help" && args[2] == "main" ) || help_all {
        begin_doc!("main");
        ...
        end_escape!();
        print( "This help page catalogs all the enclone help pages.  We strongly \
        ...
        print( "\\boldblue{enclone is part of the 10x Genomics immune profiling tools, including \
        ...
        ldoc_red!( "enclone help command", "info about enclone command line argument processing" );
        ldoc_red!( "enclone help glossary", "glossary of terms used by enclone, and conventions" );
        ldoc_red!( "enclone help example1", "explanation of an example" );
        doc_red!( "enclone help example2", "example showing gene expression \
            and feature barcodes (wide)" );
        ldoc_red!( "enclone help support", "how we can help, enclone@10xgenomics.com" );
        ldoc!( "enclone help input", "how to provide input to enclone" );
        doc!( "enclone help input_tech", "how to provide input to enclone (technical notes)" );
        ...
        doc!( "enclone help amino", "per chain column options for amino acids" );
        doc!( "enclone help display", "other clonotype display options" );
        ldoc!( "enclone help indels", "insertion and deletion handling" );
        ldoc!( "enclone help color", "how enclone uses color, and related things" );
        doc!( "enclone help ideas", "ideas for features that might be implemented" );
        doc!( "enclone help faq", "frequently asked questions (long)" );
        doc!( "enclone help all", "concatenation of all the help pages (long, wide);" );
    ...
    if ( args.len() == 3 && args[1] == "help" && args[2] == "how" ) || help_all {
        ...
        end_escape!();
        print( "The goal of enclone is to find and display the clonotypes within single cell \
        ...
        print_with_box( "1. It is extremely easy to get false positives: the incorrect \
        ...
        print( "To address these challenges, the enclone algorithm has several steps, which we \
            briefly outline:\n\n" );
        print_with_box( "1. For samples from a given donor, enclone first \
    ...
    if ( args.len() == 3 && args[1] == "help" && args[2] == "command" ) || help_all {
        ...
        print( "• Before processing its command line, enclone first checks for environment\n\
        ...
        print( "\\bold{2. Color}\n\n" );
        print_enclone(plain);
        print( " uses ANSI escape codes for color and bolding, frivolously, for emphasis, \
        ...
        print( "• The effect of this will be that you'll see only the first screen of output.  \
        ...
        print( "• If for whatever reason you need to turn off output piping, add the argument \
    ...
    if ( args.len() == 3 && args[1] == "help" && args[2] == "glossary" ) || help_all {
        ...
        doc!( "clone", "a cell in a clonotype, or in an exact subclonotype" );

        // doc onesie etc.

        ldoc!( "onesie", "a clonotype or exact subclonotype having exactly one chain" );
        doc!( "twosie", "a clonotype or exact subclonotype having exactly two chains" );
        doc!( "threesie", "a clonotype or exact subclonotype having exactly three chains;" );
        doc!( "", "these frequently represent true biological events, arising from expression" );
        doc!( "", "of both alleles" );
        doc!( "foursie", "a clonotype or exact subclonotype having exactly four chains;" );
        ...
        doc!( "dataset", "all sequencing data obtained from a particular library type" );
        doc!( "", "(e.g. TCR or BCR or GEX or FB), from one cell group, processed by running" );
        ...
        println!( "conventions\n" );
        end_escape!();
        println!( "• Zero or one?  We number exact subclonotypes as 1, 2, ... and likewise with\n\
