...
pub fn graph_filter(mut tig_bc: &mut Vec<Vec<TigData>>, graph: bool) {
    ...
    for i in 0..tig_bc.len() {
        for j1 in 0..tig_bc[i].len() {
            if tig_bc[i][j1].left {
                let x1 = &tig_bc[i][j1];
                let p1 = bin_position( &seqs, 
                    &(x1.seq.clone(), true, x1.cdr3_aa.clone(), x1.v_ref_id) ) as usize;
                for j2 in 0..tig_bc[i].len() {
                    if !tig_bc[i][j2].left {
                        let x2 = &tig_bc[i][j2];
                        let p2 = bin_position( &seqs, 
                            &(x2.seq.clone(), false, x2.cdr3_aa.clone(), x2.v_ref_id) ) as usize;
    ...
    for v in 0..g.node_count() {
        if g.n_to(v) > 1 {
            ...
            for i in 0..g.n_to(v) {
                ...
                fwriteln!( log, "• heavy chain {} = {}, weight = {}/{}", 
                    w, seqs[w].2, numi, ncells );
                    ...
                    if numi_best >= MIN_RATIO_KILL * max( 1, numi ) && numi <= MAX_KILL {
                        ...
                        if ncells <= MAX_KILL_CELLS {
                            ...
                            if graph {
                                ...
                                println!( "killed by {} to {}, ncells = {}, numi = {}",
                            ...
                            for j in 0..g.n_from(w) {
                                ...
                                if vx != v && ncellsx >= MIN_RATIO_KILL * ncells
    ...
    for v in 0..g.node_count() {
        if g.n_from(v) > 1 {
            ...
            for i in 0..g.n_from(v) {
                ...
                fwriteln!( log, "• light chain {} = {}, weight = {}/{}", 
                    w, seqs[w].2, weight.1, weight.0 );
                stats.push( (*weight, w) );
            }
            reverse_sort(&mut stats);
            for i in 1..stats.len() {
                if (stats[0].0).0 >= MIN_RATIO_KILL_HEAVY * max( 1, (stats[i].0).0 )
                    && (stats[i].0).0 <= MAX_KILL_HEAVY && (stats[i].0).1 <= MAX_KILL_HEAVY_CELLS {
