...
pub fn help3( args: &Vec<String> ) {
    ...
    */
    ...
    if ( args.len() == 3 && args[1] == "help" && args[2] == "input_tech" ) || help_all {
        ...
        end_escape!();
        print( "enclone only uses certain files, which are all in the outs subdirectory of \
        ...
        // end_escape!();
        print( "Note that you must use the output of Cell Ranger version \\boldred{≥ 3.1}.  There \
    ...
    if ( args.len() == 3 && args[1] == "help" && args[2] == "parseable" ) || help_all {
        ...
        end_escape!();
        println!( "specifying the name of the file that is to be written to.\n\
        ...
        log.append( &mut b"details below\n".to_vec() );
        if !plain {
            emit_bold_escape(&mut log);
            emit_red_escape( &mut log );
        }
        ...
        end_escape!();
        doc!( "clonotype_id", "identifier of clonotype within the clonotype group = 0, 1, ..." );
        ldoc!( "clonotype_ncells", "total number of cells in the clonotype" );
        ldoc!( "nchains", "total number of chains in the clonotype" );
        print_tab2(&rows);

        rows.clear();
        print( "\\bold{3. per chain fields, where <i> is 1,2,... (see above)\n\
        ...
        ldoc!( "var_indices_dna<i>", "DNA positions in chain that vary across the clonotype" );
        doc!( "var_indices_aa<i>", "amino acid positions in chain that vary across the clonotype" );
        doc!( "share_indices_dna<i>", "DNA positions in chain that are constant across the \
            clonotype," );
        doc!( "", "but differ from the donor ref" );
        doc!( "share_indices_aa<i>", "amino acid positions in chain that are constant across the \
            clonotype," );
        ...
        doc!( "exact_subclonotype_id", "identifer of exact subclonotype = 1, 2, ..." );
        ldoc!( "barcodes", "comma-separated list of barcodes for the exact subclonotype" );
        ldoc!( "In addition, there is a field for each lead variable, as documented at",
            "\\ext" );
        doc!( "\"enclone help lvars\", which has a default value, that may be modified using",
            "\\ext" );
        doc!( "the LVARS and LVARSP options.", "\\ext" );
        print_tab2(&rows);

        rows.clear();
        print( "\\bold{5. per chain, per exact subclonotype fields, where <i> is 1,2,... \
            (see above)}\n\n" );
        println!( "[all apply to chain i of a particular exact clonotype]\n" );
        doc!( "vj_seq<i>", "DNA sequence of V..J" );
        doc!( "seq<i>", "full DNA sequence" );
        doc!( "q<n>_<i>", "special option to display a comma-separated list of the quality" );
        doc!( "", "scores for chain i, at zero-based position n, numbered starting at the" );
        doc!( "", "beginning of the V segment, for each cell in the exact subclonotype" );
        ldoc!( "v_start<i>", "start of V segment on full DNA sequence" );
        ldoc!( "const_id<i>", "numerical identifier of constant region (or null, if not known)" );
        ldoc!( "utr_id<i>", "numerical identifier of 5'-UTR region (or null, if not known)" );
        doc!( "utr_name<i>", "name of 5'-UTR region (or null, if not known)" );
        ldoc!( "cdr3_start<i>", "base position start of CDR3 sequence on full contig" );
        doc!( "cdr3_aa<i>", "amino acid sequence of CDR3" );
        ldoc!( "var_aa<i>", 
            "amino acids that vary across the clonotype (synonymous changes included)" );
        ldoc!( "In addition, per chain, there is a field for each variable as described at",
            "\\ext" );
        doc!( "\"enclone help cvars\", after suffixing with <i>, and depending on specification", 
            "\\ext" );
    ...
    if ( args.len() == 3 && args[1] == "help" && args[2] == "filter" ) || help_all {
        ...
        doc!( "MIN_CELLS=n", "only show clonotypes having at least n cells" );
        doc!( "MAX_CELLS=n", "only show clonotypes having at most n cells" );
        doc!( "CELLS=n", "only show clonotypes having exactly n cells" );

        // doc MIN_UMIS

        ldoc!( "MIN_UMIS=n", "only show clonotypes having ≳ n UMIs on some chain on some cell" );

        // doc *CHAINS

        ldoc!( "MIN_CHAINS=n", "only show clonotypes having at least n chains" );
        doc!( "MAX_CHAINS=n", "only show clonotypes having at most n chains" );
        doc!( "CHAINS=n", "only show clonotypes having exactly n chains" );

        // doc CDR3

        ldoc!( "CDR3=<pattern>", "only show clonotypes having a CDR3 amino acid seq that matches" );
        doc!( "", "the given pattern (regular expression)*, from beginning to end" );
        ...
        ldoc!( "MIN_EXACTS=n", "only show clonotypes having at least n exact subclonotypes" );

        // doc VJ

        ldoc!( "VJ=seq", "only show clonotypes using exactly the given V..J sequence" );
        doc!( "", "(string in alphabet ACGT)" );

        // doc MIN_DATASETS

        ldoc!( "MIN_DATASETS=n", "only show clonotypes containing cells from at least n datasets" );

        // doc CDIFF

        ldoc!( "CDIFF", "only show clonotypes having a difference in constant region with the" );
        ...
        let mut rows = Vec::<Vec<String>>::new();
        rows.push( vec![ "CDR3=CARPKSDYIIDAFDIW".to_string(), 
            "have exactly this sequence as a CDR3".to_string() ] );
        rows.push( vec![ "CDR3=\"CARPKSDYIIDAFDIW|CQVWDSSSDHPYVF\"".to_string(),
            "have at least one of these sequences as a CDR3".to_string() ] );
        rows.push( vec![ "CDR3=\".*DYIID.*\"".to_string(), 
            "have a CDR3 that contains DYIID inside it".to_string() ] );
        log.clear();
        print_tabular_vbox(&mut log, &rows, 2, &b"l|l".to_vec(), false);
        println!( "{}", log );
        println!( "Note that double quotes should be used if the pattern \
            contains characters other than letters.\n" );
        println!( "A gentle introduction to regular expressions may be found at\n\
