...
pub fn join_exacts(
    ...
    for i in 0..exact_clonotypes.len() {
        for j in 0..exact_clonotypes[i].clones.len() {
            ...
            if !to_bc.contains_key( &(x.lena_index,i) ) {
                ...
                to_bc.get_mut( &(x.lena_index,i) ).unwrap().push(x.barcode.clone());
            }
        }
    }
    ...
    results.par_iter_mut().for_each(|r| {
        ...
        join_core( is_bcr, i, j, &ctl, &exact_clonotypes, &info, &to_bc, &sr, &mut eq, &mut pot );
        ...
        for pj in 0..pot.len() {
            ...
            *joins += 1;
            ...
            fwriteln!( log, "\n[{}] = {}.clono{} * {}.clono{}", diffs, 
                lena1.iter().format(","), info[k1].clonotype_id, lena2.iter().format(","), 
            ...
            for l1 in info[k1].origin.iter() {
                fwriteln!( log, "{} = {}", 
                    ctl.sample_info.dataset_id[*l1], ctl.sample_info.descrips[*l1] );
            }
            for l2 in info[k2].origin.iter() {
                fwriteln!( log, "{} = {}", 
                    ctl.sample_info.dataset_id[*l2], ctl.sample_info.descrips[*l2] );
            }
            ...
            for u in 0..nrefs {
                fwrite!( log,
                    "versus ref {}, shares = {}, indeps = {}", u+1, shares[u], indeps[u] );
                fwriteln!( log, " (shares breakdown = V{}, J{}; V{}, J{})", 
                    shares_details[u][0], shares_details[u][1], 
                    shares_details[u][2], shares_details[u][3] );
                fwriteln!( log, 
                    "              share pos v = {}", share_pos_v[u].iter().format(",") );
            }
            if cd < 0 {
