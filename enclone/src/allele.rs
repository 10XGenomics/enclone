...
pub fn find_alleles(
    ...
    let mut allxy = vec![Vec::<(usize,Vec<u8>,Vec<usize>,usize,usize)>::new(); refdata.refs.len()];
                ...
                if !partner.is_empty() {
                    if y.seq_del.len() >= refdata.refs[id].len() - ctl.heur.ref_v_trim {
                        for l in 0..x.clones.len() {
                            ...
                            allxy[id].push( (donor, y.seq_del.clone(), partner.clone(),
                                m, x.clones[l][j].lena_index) );
                        }
                    }
                }
            }
    ...
    for id in 0..refdata.refs.len() {
        ...
        for di in 0..alls.len() {
            ...
            for p in 0..refdata.refs[id].len() - ctl.heur.ref_v_trim {
                ...
                if ( freqs.len() > 0 && freqs[0].0 >= ctl.allele_alg_opt.min_alt
                        && freqs[0].3 != c )
                    || ( freqs.len() > 1 && ctl.allele_alg_opt.min_mult * freqs[1].0 >= bases.len() 
            ...
            if ctl.allele_print_opt.con_trace {
                println!( "di = {}, id = {}, transcript = {}, ps = {}",
                    di, id, refdata.transcript[id], ps.iter().format(",") );
            }
            ...
            while i < types.len() {
                ...
                if (j-i >= ctl.allele_alg_opt.min_alt 
                    && j-i >= types.len()/ctl.allele_alg_opt.min_mult) || is_ref {
    ...
    if ctl.comp {
        println!( "used {:.2} seconds used finding alt alleles, peak mem = {:.2} GB", 
            elapsed(&ta), peak_mem_usage_gb() );
    }
alt_refs.sort();
alt_refs
}
...
pub fn sub_alts( ctl: &EncloneControl, alt_refs: &Vec<(usize,usize,DnaString)>,
    info: &mut Vec<CloneInfo>, exact_clonotypes: &mut Vec<ExactClonotype> ) {
    let t = Instant::now();
    for i in 0..info.len() {
    ...
    if ctl.comp {
        println!( "used {:.2} seconds used substituting alt alleles, peak mem = {:.2} GB", 
