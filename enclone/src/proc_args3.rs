...
pub fn proc_xcr( f: &str, gex: &str, have_gex: bool, internal_run: bool, 
    ...
    if val == "".to_string() {
        eprintln!("\nYou can't write {} with no value on the right hand side.", f);
                    ...
                    if !path_exists(&pg.rev_before("/outs")) {
                        if ctl.gen_opt.pre != "".to_string() {
                            eprintln!( "\nThe value given for {} on the enclone command line \
                                ...
                                {},\n\
                                and that path does not exist.\n", 
                                f.before("="), pg0, pg.rev_before("/outs") );
                        } else {
                            eprintln!( "\nThe value given for {} on the enclone command line \
                                includes\n{}, and that path does not exist.\n", 
                                f.before("="), pg.rev_before("/outs") );
                        }
                        std::process::exit(1);
                    }
                    if !path_exists(&p) {
...
pub fn proc_meta( f: &str, ctl: &mut EncloneControl ) {
    ...
    for (count,line) in f.lines().enumerate() {
        ...
        if count == 0 {
            ...
            let allowed_fields = vec![ "bcr".to_string(), "donor".to_string(), 
                "gex".to_string(), "sample".to_string(), "tcr".to_string() ];
            ...
            if dp.is_none() {
                ...
                ctl.sample_info.dataset_list.push( 
                    vec![ ctl.sample_info.descrips.len() ] );
            }
