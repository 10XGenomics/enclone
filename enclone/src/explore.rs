...
// best definition of exact subclonotype.
...
pub fn explore( li: usize, tig_bc: &Vec<Vec<TigData>>, ctl: &EncloneControl ) {
    if ctl.gen_opt.exp {
        ...
        while r < tig_bc.len() {
            ...
            if s > r {
                if s - r > 1 { // if only one barcode, should save but not analyze
                    ...
                    */
                    ...
                    for m in 0..tig_bc[r].len() {
                        fwrite!( log, "{} = {} = {} = {}", m+1, tig_bc[r][m].cdr3_aa, 
                            tig_bc[r][m].cdr3_dna, tig_bc[r][m].len );
                        if m < tig_bc[r].len() - 1 {
                            fwriteln!( log, " " );
                        }
                    }
                    ...
                    for m in 0..tig_bc[r].len() { // go through the chains
                        for p in 0..tig_bc[r][m].len { // go through positions
                            ...
                            for u in r..s {
                                bqb.push( 
                                    ( tig_bc[u][m].seq[p], tig_bc[u][m].quals[p], u-r) );
                            }
                            bqb.sort();
                    ...
                    if printme || ctl.gen_opt.weak {
                        ...
                        for j in 0..bc_print.len() {
                            ...
                            fwriteln!( log, "bc {} = {}", bc_id,
                                tig_bc[bc_id + r][0].barcode );
                        }
