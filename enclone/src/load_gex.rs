...
pub fn load_gex( ctl: &mut EncloneControl,
    ...
    let comp = ctl.comp;
    let mut results = Vec::<
        (usize, Vec<String>, Vec<String>, MirrorSparseMatrix, Option<f64>, Option<f64>)>::new();
    for i in 0..ctl.sample_info.gex_path.len() {
        results.push( (i, Vec::<String>::new(), Vec::<String>::new(),
            MirrorSparseMatrix::new(), None, None) );
    }
    ...
    results.par_iter_mut().for_each(|r| {
        ...
        if gex_outs[i].len() > 0 {
            ...
            if comp {
                println!( "-- used {:.2} seconds getting reads per cell metric", elapsed(&tm) );
            }
            if rpc.len() > 0 {
                if rpc.parse::<f64>().is_err() {
                    eprintln!( 
                        "\nIn load_gex, failed to parse metric reads_per_cell from {}.\n", json );
                    eprintln!( "This is unexpected behavior, if you can't figure out what \
            ...
            if comp {
                println!( "-- used {:.2} seconds reading features and barcodes", elapsed(&tfb) );
            }
            let tfb = Instant::now();
            if path_exists(&format!("{}/GRCh38", dir)) {
            ...
            if !ctl.gen_opt.h5 {
                if path_exists(&bin_file) {
                    ...
                    load_feature_bc_matrix( 
                        &gex_outs[i], &mut features, &mut barcodes, &mut matrix );
...
pub fn get_gex_info( mut ctl: &mut EncloneControl ) -> GexInfo {
    ...
    load_gex( &mut ctl, &mut gex_features, &mut gex_barcodes, &mut gex_matrices, &mut gex_mults,
        &mut fb_mults );
